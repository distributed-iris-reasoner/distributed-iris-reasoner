...
public class Rdfs7Test extends CascadingTest {
	...
	protected  void createFacts() throws IOException {
		...
		if (enableCluster) {
			new Importer().importFromFile(defaultConfiguration, defaultConfiguration.project, this.getClass().getResource("/facts/rdfs7.nt").getPath(), "import");
		} else {
			new Importer().processNTriple(defaultConfiguration, this.getClass().getResource("/facts/rdfs7.nt").getPath(), defaultConfiguration.project, "import");
		}
	}
