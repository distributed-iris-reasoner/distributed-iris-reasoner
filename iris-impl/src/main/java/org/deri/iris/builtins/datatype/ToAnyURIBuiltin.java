 ...
 * type instances to {@link IAnyURI} instances. The following data types are
 * supported:
 * <ul>
 * <li>AnyURI</li>
 * <li>String</li>
 * </ul>
 */
public class ToAnyURIBuiltin extends ConversionBuiltin {
	 ...
	 */
	public ToAnyURIBuiltin(ITerm... terms) {
		super(PREDICATE, terms);
	}

	@Override
	protected IAnyURI convert(ITerm term) {
		if (term instanceof IAnyURI) {
			return (IAnyURI) term;
		} else if (term instanceof IStringTerm) {
			return toAnyURI((IStringTerm) term);
		}
	...
	/**
	 * Converts a {@link IStringTerm} term to a {@link IAnyURI} term.
	 * 
	 * @param term The {@link IStringTerm} term to be converted.
	 * @return A new {@link IAnyURI} term representing the result of the
	 *         conversion, or <code>null</code> if the conversion fails.
	 */
	public static IAnyURI toAnyURI(IStringTerm term) {
		try {
			URI uri = URI.create(term.getValue());
