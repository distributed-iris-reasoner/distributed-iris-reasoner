...
public class ExactEqualBuiltinTest extends TestCase {
	...
	public void testEvaluation() throws Exception{
		...
		assertNotNull("10.0d should be exactly equal to 10.0d", xy.evaluate(
						BASIC.createTuple(CONCRETE.createDouble(10), CONCRETE.createDouble(10))));
		
		assertNotNull("10.0f should be exactly equal to 10.0f", xy.evaluate(
						BASIC.createTuple(CONCRETE.createFloat(10), CONCRETE.createFloat(10))));
		
		assertNotNull("+0.0d should be exactly equal to -0.0d", xy.evaluate(
						BASIC.createTuple(CONCRETE.createDouble(+0.0d), CONCRETE.createDouble(-0.0d))));
		
		assertNotNull("+0.0f should be exactly equal to -0.0f", xy.evaluate(
						BASIC.createTuple(CONCRETE.createFloat(+0.0f), CONCRETE.createFloat(-0.0f))));
		
		assertNull("5 should not be exactly equal to 5.0", xy.evaluate(
					BASIC.createTuple(CONCRETE.createInteger(5), CONCRETE.createDouble(5))));
		
		assertNull("5.0f should not be exactly equal to 5.0d", xy.evaluate(
						BASIC.createTuple(CONCRETE.createFloat(5), CONCRETE.createDouble(5))));
			
		assertNull("5 should not exactly equal to 2", xy.evaluate(
					BASIC.createTuple(CONCRETE.createInteger(2), CONCRETE.createInteger(5))));
		
		assertNull("5 should not be exactly equal to a", xy.evaluate(
